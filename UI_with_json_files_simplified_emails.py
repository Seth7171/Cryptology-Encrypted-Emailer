...
class EmailSenderGUI(QWidget):
    ...
    def triggerSendMessage(self):
        ...
        # TODO: continue from here: turn key back to bytes, decrypt message with blowfish and display it, display subject, consider moving to loadEmails function...
                    ...
                    try:
                        with open(file_path, 'r') as f:
                            ...
                            if email.get('to') == self.user:
                                # add verify and decryption
                                message = email.get('msg')

                                if self.rabin_verify(email.get('dig_sign_blowfish_message'), message):
                                    if env.debug:
                                        print("signature valid")

                                    email_entry = f"(secure) {email.get('subject', 'No Subject')} - from {email.get('from', 'Unknown')}"
                                else:
                                    # if env.debug:
    ...
    def display_email_details(self, item):
        ...
        try:
            with open(file_path, 'r') as file:
                ...
                print(decrypted_blowfish_key)

                self.subjectLineEdit.setText(email.get('subject', ''))
                self.bodyTextEdit.setPlainText(email.get('msg', ''))
    ...
    def load_other_user_credentials(self):
        ...
        try:
            with open(credentials_path, 'r') as file:
                ...
                # TODO: fix same public key for bob and alice
