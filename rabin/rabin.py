...
# return true if an is a witness, false otherwise
def witness(a, k, m, p):
    b = pow(a, m, p)
    if b == 1:
        return False
...
# returns a random odd integer of 'bits' bit length.
...
# returns a probable prime of 'bits' bit length using miller-rabin test
def get_prime(bits):
...
# p = 
# q = 
